...
public class userdata implements Initializable {
        ...
        Tphone.setOnKeyPressed(new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                if (event.getCode().equals(KeyCode.ENTER)) {
                    fetchِAuto(Long.parseLong(Tphone.getText()));
                }
            }
        });
    }
    ...
    public void fetchِAuto(long id){
        ResultSet dbResAllTotal;
        try {
            String sqlscript = "SELECT * from kunafahut.orderdetails where clientPhone = " + id + ";";
    ...
    public static Node getPrintableText(){
        ...
        stringBuilder.append("                     حاتي المصطفي                          \n");
        stringBuilder.append("               01111470474 / 01066865067                     \n");
        stringBuilder.append("-------------------------------------------------" + "\n");
        stringBuilder.append("          رقم الاوردر   :                    "+idgenerate+" \n");
        stringBuilder.append("    الوفت والتاريخ:  "+orderTime+" \n");
        if(!username.equals("")&&!userlocation.equals("")&&userphone !=0){
            stringBuilder.append("-------------------------------------------------" + "\n");
            stringBuilder.append("اسم العميل   :    "+username+"\n");
            stringBuilder.append("         رقم التليفون   :    "+"0"+userphone+"\n");
        ...
        try {
            ...
            while (dbResGetId.next()){
                ...
                String description = dbResGetId.getString("description");
                stringBuilder.append("=================================\n");
                stringBuilder.append( type+" "+name+" \t "+no+" "+quantity+" \t "+discount+" \t "+total+" * \n");
                if (!description.equals("null")){
                    if (description.length() > 35){
                        stringBuilder.append(description.substring(0,34)+"\n");
                        stringBuilder.append(description.substring(34)+"\n");
                    }else {
        ...
        stringBuilder.append("--------------------------------------------------\n");
        stringBuilder.append( "                   كبابجي بدرجة دكتور                      "+"\n");
        stringBuilder.append( "            شارع عين شمس أمام سوق النهضة                "+"\n");
