...
public class ordersConfig implements Initializable {
    ...
    public void preEditing(int typed){
        ...
        String sendOrderData = "insert into preorder ( type, name, no, quantity, price, disc, netPrice, description)\n" +
                "select type, name, no, quantity, price, disc, netPrice, description from ordersdata WHERE `orderNo` ="+typed+";";
        String sqlscript2 = "DELETE FROM `kunafahut`.`ordersdata` WHERE `orderNo` ="+typed+";";
        try {
