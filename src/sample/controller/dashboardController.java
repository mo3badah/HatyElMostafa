...
public class dashboardController implements Initializable{
    ...
    private void loadMainData(){
        try{
            String sqlscriptDay = "select count(orderNo) as num, sum(totPrice) as price from kunafahut.orderdetails where DATE(orderTime) = CURDATE()";
            String sqlscriptWeek = "select count(orderNo) as num, sum(totPrice) as price from kunafahut.orderdetails WHERE  YEARWEEK(`orderTime`, 1) = YEARWEEK(CURDATE(), 1)";
            String sqlscriptMonth = "select count(orderNo) as num, sum(totPrice) as price from kunafahut.orderdetails where (orderTime between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW() );";
